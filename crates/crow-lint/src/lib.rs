...
impl<'hir> LintCtxt<'hir> {
    ...
    pub fn expr(&self, id: ExprId) -> &'hir HirExpr { self.body.expr(id) }
    pub fn stmt(&self, id: StmtId) -> &'hir HirStmt { self.body.stmt(id) }
    pub fn pat(&self, id: PatId) -> &'hir HirPat { self.body.pat(id) }
}
...
impl LintDriver {
    pub fn new(passes: Vec<Box<dyn LintPass>>) -> Self {
        Self { passes, diags: Vec::new() }
    }

    pub fn run(mut self, hir: &Hir, tycx: & TypeckResults) -> Vec<LinterWarnings> {
    ...
    fn visit_item(&mut self, hir: &Hir, item: &HirItem, tycx: & TypeckResults) {
        let body_id = match &item.kind {
            HirItemKind::Fun(f) => Some(f.body),
            HirItemKind::Const(c) => Some(c.body),
            HirItemKind::Struct(_) | HirItemKind::Enum(_) | HirItemKind::Native(_) => None,
        };

        if let Some(bid) = body_id {
            let body = hir.body(bid);
            let mut cx = LintCtxt { diags: Vec::new(), body, hir_types: tycx };

            for p in &mut self.passes { p.enter_item(&mut cx, item); }
            for p in &mut self.passes { p.enter_body(&mut cx, body); }

            self.visit_expr(&mut cx, body.root_expr);

            for p in &mut self.passes { p.exit_body(&mut cx, body); }
            for p in &mut self.passes { p.exit_item(&mut cx, item); }
    ...
    fn visit_expr(&mut self, cx: &mut LintCtxt, id: ExprId) {
        ...
        for p in &mut self.passes { p.enter_expr(cx, expr); }
        ...
        match &expr.kind {
            ...
            HirExprKind::If(cond, then, els) => {
                ...
                if let Some(e) = els { self.visit_expr(cx, *e); }
            }
            HirExprKind::Call(callee, args) => {
                self.visit_expr(cx, *callee);
                for &a in args { self.visit_expr(cx, a); }
            }
            HirExprKind::Match { subjects, arms } => {
                for &s in subjects { self.visit_expr(cx, s); }
                for arm in arms {
                    for p in &mut self.passes { p.enter_arm(cx, arm); }
                    for &pid in &arm.pats { self.visit_pat(cx, pid); }
            ...
            HirExprKind::Diverge(_, arg) => {
                if let Some(a) = arg { self.visit_expr(cx, *a); }
            }
            HirExprKind::Lit(_) | HirExprKind::Var(_) => {}
        }

        let expr = cx.body.expr(id);
        for p in &mut self.passes { p.exit_expr(cx, expr); }
    }

    fn visit_stmt(&mut self, cx: &mut LintCtxt, id: StmtId) {
        let stmt = cx.body.stmt(id);
        for p in &mut self.passes { p.enter_stmt(cx, stmt); }

        match &stmt.kind {
            HirStmtKind::Variable { init, .. } => self.visit_expr(cx, *init),
            HirStmtKind::WildcardAssign { init, .. } => self.visit_expr(cx, *init),
    ...
    fn visit_pat(&mut self, cx: &mut LintCtxt, id: PatId) {
        ...
        for p in &mut self.passes { p.enter_pat(cx, pat); }

        match &pat.kind {
            HirPatKind::Unpack(_, children) => {
                for &c in children { self.visit_pat(cx, c); }
            }
            HirPatKind::Or(alts) => {
                for &a in alts { self.visit_pat(cx, a); }
            }
            _ => {}
        }
    }
    ...
    fn enter_stmt(&mut self, cx: &mut LintCtxt, stmt: &HirStmt) {
        if let HirStmtKind::Expr(eid) = &stmt.kind {
            ...
            if matches!(expr.kind, HirExprKind::Call(id, ..)) {
                ...
                cx.warn(LinterWarnings::UnusedResult { stmt: format!("{:?}", expr), src: stmt.span.0.clone().into(), span: stmt.span.1.clone().into() });
            }
        }
    }
}
...
impl UnusedVariableLint {
    pub fn new() -> Self {
        Self { declared: HashMap::new(), used: HashSet::new() }
    }
}

impl LintPass for UnusedVariableLint {
    fn id(&self) -> LintId { LintId::UnusedVariable }
    ...
    fn enter_item(&mut self, _cx: &mut LintCtxt, item: &HirItem) {
        if let HirItemKind::Fun(f) = &item.kind {
            for param in &f.params {
                self.declared.insert(param.local_id, (param.name.clone(), param.span.clone()));
            }
        }
    }

    fn enter_stmt(&mut self, _cx: &mut LintCtxt, stmt: &HirStmt) {
        if let HirStmtKind::Variable { local_id, name, .. } = &stmt.kind {
            self.declared.insert(*local_id, (name.clone(), stmt.span.clone()));
        }
    }

    fn enter_expr(&mut self, _cx: &mut LintCtxt, expr: &HirExpr) {
        if let HirExprKind::Var(crow_resolving::table::Res::Local(lid)) = &expr.kind {
    ...
    fn exit_body(&mut self, cx: &mut LintCtxt, _body: &HirBody) {
        for (lid, (name, span)) in &self.declared {
            if !self.used.contains(lid) && !name.starts_with('_') {
                cx.warn(LinterWarnings::UnusedVariable { val_name: name.clone(), src: span.0.clone().into(), span: span.1.clone().into() });
            }
        }
    }
}
...
impl LintPass for UnreachableCodeLint {
    fn id(&self) -> LintId { LintId::UnreachableCode }
    ...
    fn enter_expr(&mut self, cx: &mut LintCtxt, expr: &HirExpr) {
        if let HirExprKind::Block(stmt_ids) = &expr.kind {
            ...
            for &sid in stmt_ids {
                ...
                if diverged {
                    cx.warn(LinterWarnings::UnreachableCode { src: stmt.span.0.clone().into(), span: stmt.span.1.clone().into() });
                    break;
                }
                if let HirStmtKind::Expr(eid) = &stmt.kind {
                    let e = cx.expr(*eid);
...
impl UnusedMutLint {
    pub fn new() -> Self {
        Self { mut_locals: HashMap::new(), assigned: HashSet::new() }
    }
}

impl LintPass for UnusedMutLint {
    fn id(&self) -> LintId { LintId::UnusedMut }
    ...
    fn enter_stmt(&mut self, _cx: &mut LintCtxt, stmt: &HirStmt) {
        if let HirStmtKind::Variable { local_id, name, mutable: true, .. } = &stmt.kind {
            self.mut_locals.insert(*local_id, (name.clone(), stmt.span.clone()));
        }
    }
    ...
    fn enter_expr(&mut self, cx: &mut LintCtxt, expr: &HirExpr) {
        if let HirExprKind::Assign(lhs_id, _) = &expr.kind {
            ...
            if let HirExprKind::Var(crow_resolving::table::Res::Local(lid)) = &lhs.kind {
    ...
    fn exit_body(&mut self, cx: &mut LintCtxt, _body: &HirBody) {
        for (lid, (name, span)) in &self.mut_locals {
            if !self.assigned.contains(lid) {
                cx.warn(LinterWarnings::UnusedMut { var_name: name.clone(), src: span.0.clone().into(), span: span.1.clone().into() });
            }
        }
    }
}

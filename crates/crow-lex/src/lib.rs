...
use crow_common::{bail, span::Span};
...
impl<'s> Lexer<'s> {
    ...
    fn advance_number(&mut self) -> Token {
        ...
        // non-digit char.
        while self.is_ascii_digit() && !self.is_eof() {
            buffer.push(self.current.unwrap());
            self.advance();
    ...
    /// Advances hex number
    fn advance_hex_number(&mut self) -> Token {
        // Bumping `0x`
        let start = self.idx;
        self.advance();
        self.advance();
        ...
        // Building number before reaching
        // non-digit char.
        while self.is_digit(16) && !self.is_eof() {
            buffer.push(self.current.unwrap());
            self.advance();
        }
        let end = self.idx;
    ...
    /// Advances binary number
    fn advance_bin_number(&mut self) -> Token {
        // Bumping `0b`
        let start = self.idx;
        self.advance();
        self.advance();
        ...
        // Building number before reaching
        // non-digit char.
        while self.is_digit(2) && !self.is_eof() {
            buffer.push(self.current.unwrap());
            self.advance();
        }
        let end = self.idx;
    ...
    /// Advances octal number
    fn advance_octal_number(&mut self) -> Token {
        // Bumping `0o`
        let start = self.idx;
        self.advance();
        self.advance();
        ...
        // Building number before reaching
        // non-digit char.
        while self.is_digit(8) && !self.is_eof() {
            buffer.push(self.current.unwrap());
            self.advance();
        }
        let end = self.idx;
        ...
        match (self.current, self.next) {
            ...
            (Some(ch), next) => {
                if self.is_ascii_digit() {
                    Some(match next {
                        Some('x') => self.advance_hex_number(),
                        Some('b') => self.advance_bin_number(),
                        Some('o') => self.advance_octal_number(),
                        _ => self.advance_number(),
                    })
